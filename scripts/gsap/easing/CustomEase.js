...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("easing.CustomEase", ["easing.Ease"], function(Ease) {
		...
		var _numbersExp = /(?:(-|-=|\+=)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/ig,
			...
			CustomEase = function (name, data, config) {
		...
		CustomEase.create = function (name, data, config) {
			return new CustomEase(name, data, config);
		};

		CustomEase.version = "0.1.0";

		CustomEase.bezierToPoints = _bezierToPoints;
		CustomEase.get = function (name) {
			return Ease.map[name];
		};
